...
SEARCH_BASE = 'https://www.everlane.com/s?q={}&page={}'
DEFAULT_QUERIES = [
...
def gather_product_urls(query: str, max_pages: int = 20) -> list:
	...
	for page in range(1, max_pages + 1):
		...
		for a in soup.select('a'):
			...
			if '/products/' in href:
				if href.startswith('/'):
					page_links.append('https://www.everlane.com' + href)
				else:
...
def parse_product(url: str) -> dict:
	try:
		...
		price_el = s.select_one('[data-test*="Price"], .ProductPrice, .price')
		...
		materials = None
		for sel in ['[data-test*="Description"]','[class*="description"]','[class*="details"]']:
		...
		return {'url': url, 'name': name, 'price_raw': price_text, 'price': price_num, 'materials': materials, 'images': ','.join(dict.fromkeys(imgs))[:2000], 'brand': 'Everlane'}
	except Exception:
		return {'url': url, 'name': None, 'price_raw': None, 'price': None, 'materials': None, 'images': None, 'brand': 'Everlane'}

def scrape_everlane_queries(queries=None, max_pages: int = 20, max_workers: int = 8, out_csv: str | None = None) -> pd.DataFrame:
	if not queries:
...
def main():
	...
	ap.add_argument('--output', default='data/everlane_products.csv')
	args = ap.parse_args()
	if not os.getenv(API_KEY_ENV):
		print('Set SCRAPERAPI_KEY to run this scraper')
		return
	_ = scrape_everlane_queries(args.queries, args.max_pages, args.workers, out_csv=args.output)
